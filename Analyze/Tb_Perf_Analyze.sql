--
-- Diagnose Tibero Database
--
-- $Header: Tb_Perf_Analyze.sql v1.3 cwjeong noship $
--
-- Description : Gather Tibero statsus data from TPR
--
-- Change History
-- 2017/06/14 Initial
...
select name name, dbid from v$database;
...
COL TBVER    NEW_VALUE TBVER    NOPRINT;
...
select instance_name instance
     ...
     , to_number(replace(version,'.',''))     tbver
  from v$instance;
...
COL TBVER    NEW_VALUE TBVER    PRINT;
...
--select 'Tb_Perf_Extract_'||'&dbname'||'_'||'&timestr'||'.out' spool_name from dual;
select 'Tb_Perf_Extract_'||'&dbname'||'.out' spool_name from dual;
...
select '<|'|| 'inst|time|' ||
       'change_cnt|db_recycle_cache_size|' ||
       'result_cache_mode|parallel_threads_per_cpu|cpu_count|memory_target|log_buffer|' ||
       'db_file_multiblock_read_count|db_keep_cache_size|db_cache_size|dbwr_cnt|' ||
       'max_session_count|memory_max_target|undo_retention|db_block_size|' ||
       'open_cursors|wthr_proc_cnt|result_cache_max_size' as lines
  from dual
;
...
select '>|'|| tm.instno ||'|'|| tm.time ||'|'|| change_cnt ||'|'||
       ...
       a11 ||'|'|| a12 ||'|'|| a13 ||'|'|| a14 ||'|'|| a15 ||'|'|| a16 ||'|'|| a17 as lines
  from (
select sn.instance_number instno
     , to_char(begin_interval_time, 'YYYYMMDD-HH24') time
     , nvl(round(max(case when name = 'DB_RECYCLE_CACHE_SIZE'         then value end)/1024/1024, 1), 0) as a01
     , nvl(      max(case when name = 'RESULT_CACHE_MODE'             then value end)              , 0) as a02
     , nvl(      max(case when name = 'PARALLEL_THREADS_PER_CPU'      then value end)              , 0) as a03
     , nvl(      max(case when name = 'CPU_COUNT'                     then value end)              , 0) as a04
     , nvl(round(max(case when name = 'MEMORY_TARGET'                 then value end)/1024/1024, 1), 0) as a05
     , nvl(round(max(case when name = 'LOG_BUFFER'                    then value end)/1024/1024, 1), 0) as a06
     , nvl(      max(case when name = 'DB_FILE_MULTIBLOCK_READ_COUNT' then value end)              , 0) as a07
     , nvl(round(max(case when name = 'DB_KEEP_CACHE_SIZE'            then value end)/1024/1024, 1), 0) as a08
     , nvl(round(max(case when name = 'DB_CACHE_SIZE'                 then value end)/1024/1024, 1), 0) as a09
     , nvl(      max(case when name = 'DBWR_CNT'                      then value end)              , 0) as a10
     , nvl(      max(case when name = 'MAX_SESSION_COUNT'             then value end)              , 0) as a11
     , nvl(round(max(case when name = 'MEMORY_MAX_TARGET'             then value end)/1024/1024, 1), 0) as a12
     , nvl(      max(case when name = 'UNDO_RETENTION'                then value end)              , 0) as a13
     , nvl(      max(case when name = 'DB_BLOCK_SIZE'                 then value end)              , 0) as a14
     , nvl(      max(case when name = 'OPEN_CURSORS'                  then value end)              , 0) as a15
     , nvl(      max(case when name = 'WTHR_PROC_CNT'                 then value end)              , 0) as a16
     , nvl(      max(case when name = 'RESULT_CACHE_MAX_SIZE'         then value end)              , 0) as a17
  from _tpr_modified_param pr
     , _tpr_snapshot sn
 where pr.thread# = sn.thread#
   and pr.snap_id = sn.snap_id
   and pr.name in ( 'DB_RECYCLE_CACHE_SIZE'
                  , 'RESULT_CACHE_MODE'
...
--------------------------------------------------------------------------------
SET TERMOUT ON
prompt Gathering.....[Parameter change history              ]  4 / &reports
SET TERMOUT OFF
...
select '>'||thrdno ||'|'|| time ||'|'|| name ||'|'|| value as lines
  from (
        select sp.thread# thrdno
             , to_char(sn.begin_interval_time, 'YYYYMMDD-HH24') time
             , name
             , case when sp.name  =  lag(sp.name ) over (order by sp.thread#, sp.name, sp.snap_id)
                     and sp.value <> lag(sp.value) over (order by sp.thread#, sp.name, sp.snap_id)
                    then             lag(sp.value) over (order by sp.thread#, sp.name, sp.snap_id) || '->' || sp.value
                    end  value
          from _TPR_MODIFIED_PARAM sp
             , _TPR_SNAPSHOT  sn
         where sp.thread#       = sn.thread#
      ...
      from (
         select instno
           ...
           from ( select instance_number instno
                       ...
                       , round(sum(case when wait_class = 'Application'    then time_waited_micro/1000    end),2) t01
                       , round(sum(case when wait_class = 'Cluster'        then time_waited_micro/1000    end),2) t02
                       , round(sum(case when wait_class = 'Commit'         then time_waited_micro/1000    end),2) t03
                       , round(sum(case when wait_class = 'Concurrency'    then time_waited_micro/1000    end),2) t04
                       , round(sum(case when wait_class = 'Configuration'  then time_waited_micro/1000    end),2) t05
                       , round(sum(case when wait_class = 'Network'        then time_waited_micro/1000    end),2) t06
                       , round(sum(case when wait_class = 'Other'          then time_waited_micro/1000    end),2) t07
                       , round(sum(case when wait_class = 'System I/O'     then time_waited_micro/1000    end),2) t08
                       , round(sum(case when wait_class = 'User I/O'       then time_waited_micro/1000    end),2) t09
