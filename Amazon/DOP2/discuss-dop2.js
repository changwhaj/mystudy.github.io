...
function setSequence(seq, vlist) {
    ...
    setCookie("DOP2"+seq, vlist, {secure: true, 'expires': expDate});
}
...
function NewTab(question_id, discuss_id) {
    ...
    if (cntrlIsPressed || seq == "X") {
        ...
        // if (discuss_id == "") {
            ...
            //     "/";
            url = "https://changwhaj.github.io/exam-assets/exam/aws/DOP_C02/DOP2-P" 
                + String(parseInt(question_id/10+0.9)) +
                ".html";
            url = "https://changwhaj.github.io/exam-assets/exam/aws/DOP_C02/DOP2-Q" + question_id + ".html"
        } else {
            url = "https://www.examtopics.com/discussions/amazon/view/" 
                + discuss_id +
                "-exam-aws-certified-solutions-architect-professional-dop-c02/";
        }
...
if (passwd == undefined) {
    ...
    if (passwd.toUpperCase() == PASSKEY) {
        ...
        expDate.setMonth(expDate.getMonth() + 1);
        expDate = expDate.toUTCString();
        setCookie('passdop2', passwd, {secure: true, 'expires': expDate});
    }
}
passwd = getCookie('passdop2');
